...
export const getUrlMap = (
  ...
  let host: string, hostRecord: string
  if (['cn_gf01', 'cn_qd01', 'prod_gf_cn', 'prod_qd_cn'].includes(server)) {
    host = 'https://api-takumi.mihoyo.com/'
    hostRecord = 'https://api-takumi-record.mihoyo.com/'
  } else if (['os_usa', 'os_euro', 'os_asia', 'os_cht'].includes(server)) {
  ...
  const urlMap = {
    genshin: {
      ...
      spiralAbyss: {
        ...
        query: `role_id=${uid}&schedule_type=${
          data.schedule_type
        }&server=${server}`
      },
      /** 角色详情 */
      ...
      blueprint: {
        ...
        query: `share_code=${data.share_code}&region=${server}`
      },
      /** 角色技能 */
      avatarSkill: {
      ...
      avatar_cardList: {
        ...
        query: `limit=999&need_action=false&need_avatar=true&need_stats=true&offset=0&role_id=${uid}&server=${server}`
      },
      /** 七圣召唤行动牌数据 */
      action_cardList: {
        url: `${hostRecord}game_record/app/genshin/api/gcg/cardList`,
        query: `limit=999&need_action=true&need_avatar=false&need_stats=true&offset=0&role_id=${uid}&server=${server}`
      },
    ...
    honkaisr: {
      ...
      ys_ledger: {
        ...
        query: `region=${server}&uid=${uid}&month=${data.month}`
      },
      /** 角色详情 */
      character: {
        url: `${hostRecord}game_record/app/hkrpg/api/avatar/info`,
      ...
      bbs_sign_info: {
        ...
        query: `act_id=e202304121516551&region=${server}&uid=${uid}`,
        sign: true
      },
      /** 签到奖励 */
      bbs_sign_home: {
      ...
      detail: {
        ...
        query: `game=hkrpg&lang=zh-cn&item_id=${data.avatar_id}&tab_from=${data.tab_from}&change_target_level=0&uid=${uid}&region=${server}`
      }
    }
  }
  if (server.startsWith('os')) {
    ...
    urlMap.genshin.bbs_sign_info.query = `act_id=e202102251931481&region=${server}&uid=${uid}`
    urlMap.genshin.bbs_sign_home.url =
      'https://hk4e-api-os.hoyoverse.com/event/sol/home'
    urlMap.genshin.bbs_sign_home.query = `act_id=e202102251931481&region=${server}&uid=${uid}`
    ...
    urlMap.genshin.detail.query = `lang=zh-cn&uid=${uid}&region=${server}&avatar_id=${data.avatar_id}`
    ...
    urlMap.genshin.blueprint.query = `share_code=${data.share_code}&region=${server}&lang=zh-cn`
    ...
    urlMap.genshin.ys_ledger.query = `lang=zh-cn&month=${data.month}&uid=${uid}&region=${server}`
    urlMap.genshin.useCdk.url =
      'https://sg-hk4e-api.hoyoverse.com/common/apicdkey/api/webExchangeCdkey'

    urlMap.genshin.useCdk.query = `uid=${uid}&region=${server}&lang=zh-cn&cdkey=${data.cdk}&game_biz=hk4e_global`
  }
  if (game === 'honkaisr' && server.includes('official')) {
    ...
    urlMap.honkaisr.bbs_sign_home.query = `act_id=e202303301540311&region=${server}&uid=${uid}&lang=zh-cn`

    urlMap.honkaisr.bbs_sign_info.url = `https://sg-public-api.hoyolab.com/event/luna/os/info`
    urlMap.honkaisr.bbs_sign_info.query = `act_id=e202303301540311&region=${server}&uid=${uid}&lang=zh-cn`
  }
